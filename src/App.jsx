...
import { 
  ...
  Share2, Check, Edit2, X, AlertTriangle, Download, Type, Navigation, LogOut, Lock, Mail, Printer, Lightbulb, UserCog
} from 'lucide-react';
...
// ใช้ Collection นี้เพื่อเก็บข้อมูล
    ...
    <div className="h-screen bg-slate-50 flex flex-col items-center justify-center p-6 animate-fade-in relative">
        ...
        <div className="relative z-10 w-full max-w-sm">
            <div className="mb-10 text-center">
                <div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce"><UserCog size={32}/></div>
...
// --- PATIENT DASHBOARD (The Core App) ---
const PatientDashboard = ({ targetUid, currentUserRole, onBack }) => {
    ...
    useEffect(() => {
        ...
        const unsubProfile = onSnapshot(doc(db, 'artifacts', APP_COLLECTION, 'users', targetUid, 'profile', 'main'), async (s) => { 
            if(s.exists()) { 
                ...
                // If profile doesn't exist yet, wait for initialization
            }
        });
    ...
    return (
        <div className="pb-24 animate-fade-in relative font-sans bg-slate-50 min-h-screen">
            {/* Header */}
            <div className="bg-white p-5 pt-10 pb-6 sticky top-0 z-30 shadow-sm flex justify-between items-center rounded-b-[32px]">
            ...
            <div className="fixed bottom-0 w-full max-w-md bg-white/90 backdrop-blur-md border-t border-slate-100 pb-6 pt-2 px-2 flex justify-between items-center z-40 rounded-t-[32px] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
                <button onClick={() => setActiveTab('home')} className={`flex-1 p-2 flex flex-col items-center gap-1 transition-all ${activeTab === 'home' ? 'text-blue-600 scale-105' : 'text-slate-400 hover:text-slate-600'}`}><Home size={24} strokeWidth={activeTab === 'home' ? 2.5 : 2}/><span className="text-[10px] font-bold">หน้าหลัก</span></button>
                <button onClick={() => setActiveTab('stats')} className={`flex-1 p-2 flex flex-col items-center gap-1 transition-all ${activeTab === 'stats' ? 'text-blue-600 scale-105' : 'text-slate-400 hover:text-slate-600'}`}><Activity size={24} strokeWidth={activeTab === 'stats' ? 2.5 : 2}/><span className="text-[10px] font-bold">กราฟ</span></button>
                <div className="relative -top-8 mx-2"><button onClick={() => setShowInputModal(true)} className="bg-gradient-to-tr from-blue-600 to-indigo-600 text-white p-4 rounded-full shadow-xl shadow-blue-300 active:scale-95 transition-transform border-4 border-slate-50 hover:shadow-2xl hover:-translate-y-1"><Plus size={32} strokeWidth={3} /></button></div>
                <button onClick={() => setActiveTab('care')} className={`flex-1 p-2 flex flex-col items-center gap-1 transition-all ${activeTab === 'care' ? 'text-blue-600 scale-105' : 'text-slate-400 hover:text-slate-600'}`}><CalendarIcon size={24} strokeWidth={activeTab === 'care' ? 2.5 : 2}/><span className="text-[10px] font-bold">ตาราง</span></button>
                <button onClick={() => setActiveTab('profile')} className={`flex-1 p-2 flex flex-col items-center gap-1 transition-all ${activeTab === 'profile' ? 'text-blue-600 scale-105' : 'text-slate-400 hover:text-slate-600'}`}><User size={24} strokeWidth={activeTab === 'profile' ? 2.5 : 2}/><span className="text-[10px] font-bold">ฉัน</span></button>
            </div>
        ...
        <div className="bg-slate-50 min-h-screen p-5 animate-fade-in relative">
            ...
            <div className="relative z-10 pt-6">
                ...
                <div className="grid gap-4">
                    {patients.map(p => (
                        <div key={p.uid} onClick={() => onSelectPatient(p.uid)} className="bg-white p-5 rounded-[28px] shadow-sm border border-slate-100 flex items-center justify-between cursor-pointer active:scale-95 transition-all hover:border-blue-200 hover:shadow-md group">
                            <div className="flex items-center gap-5">
                                <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold text-2xl shadow-lg shadow-blue-200">{p.name.charAt(0)}</div>
                                <div><h3 className="font-bold text-slate-800 text-lg group-hover:text-blue-600 transition-colors">{p.name}</h3><p className="text-sm text-slate-400">คนไข้</p></div>
                            </div>
                    ...
                    <button onClick={() => setShowAddModal(true)} className="bg-white p-5 rounded-[28px] border-2 border-dashed border-slate-200 flex items-center justify-center gap-3 text-slate-400 hover:bg-slate-50 hover:border-blue-300 hover:text-blue-500 transition-all font-bold">
                        <Plus/> เพิ่มคนไข้ใหม่
                    </button>
                </div>
                ...
                <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white p-8 rounded-[40px] w-full max-w-sm shadow-2xl">
                        <div className="text-center mb-6">
                            <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4"><UserPlus size={32}/></div>
                            <h3 className="font-bold text-xl text-slate-800 mb-1">เชื่อมต่อคนไข้</h3>
                            <p className="text-xs text-slate-500">กรอกรหัส Smart ID 6 หลักจากเครื่องคนไข้</p>
                        </div>
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (u) => { 
        ...
        if(u) {
            ...
            // else setRole(null); // Important: If no role, ensure null to show RoleSelector
        }
        setLoading(false); 
    });
  }, []);
